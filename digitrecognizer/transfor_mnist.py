...
MODEL_DIR = 'G:/数据集/inception_dec_2015/'
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        test_bottlenecks = get_test_bottlenecks(sess, test_images,
                                                jpeg_data_tensor, bottleneck_tensor)
        predictions = sess.run(prediction, feed_dict={bottleneck_input: test_bottlenecks})

        print(predictions)
        # tools.write_csv('G:/Kaggle/DigitRecognizer/inception-v3.csv', predictions)

        np.savetxt('G:/Kaggle/DigitRecognizer/submission_softmax_qy.csv',
                   np.c_[range(1, 28000), predictions],
                   delimiter=',',
